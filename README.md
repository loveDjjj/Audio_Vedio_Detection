...
这是一个小型音视频伪造检测项目，当前仓库围绕 `AV-HuBERT` 搭建了一条 `AV-Deepfake1M` `val` 子集的二分类流程。当前任务定义是 `real.mp4` 与 `fake_video_fake_audio.mp4` 的视频级分类，训练前需要先生成 split、manifest 和 mouth ROI。
...
- [scripts/train_avhubert_classifier.py](scripts/train_avhubert_classifier.py)：加载 `AV-HuBERT` backbone 和二分类头，执行训练、验证和测试。
...
- `model`：是否冻结 backbone、池化方式和分类头参数
